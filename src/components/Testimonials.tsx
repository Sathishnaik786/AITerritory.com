...
class ErrorBoundary extends React.Component<{children: React.ReactNode}, {hasError: boolean, error: any}> {
  ...
  render() {
    if (this.state.hasError) {
      return <div style={{color: 'red'}}>Testimonials Error: {String(this.state.error)}</div>;
    }
      ...
      <section className="relative w-full py-8 sm:py-16 px-0 overflow-hidden bg-transparent">
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="w-full overflow-hidden">
            ...
            >
              {console.log('Rendering duplicatedTestimonials.map', duplicatedTestimonials)}
                  ...
                  <div className="grid grid-cols-[auto_1fr] gap-3 mt-auto items-center">
                    {t.user_avatar ? (
                      <img 
                        ...
                        width={48}
                        height={48}
                        className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border-2 border-white/40 dark:border-[#2a2a40]/40 transition-opacity duration-500 ease-in-out blur-sm hover:blur-0" 
                      />
                    ) : (
