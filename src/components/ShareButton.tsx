...
interface ShareButtonProps {
  ...
  image?: string;
  className?: string;
  variant?: 'floating' | 'inline' | 'mobile' | 'dropdown';
...
export const ShareButton: React.FC<ShareButtonProps> = ({
  ...
  const handleShare = async (platformName: string) => {
    const shareData: ShareData = {
      url: addUTMParams(url, platformName),
      title,
      description,
      image
    };
    ...
    try {
      if (platformName === 'copy') {
      ...
      } else if (platformName === 'native' && isMobile && navigator.share) {
        ...
        const platform = SHARE_PLATFORMS.find(p => p.name === platformName);
        if (platform) {
      ...
      // Track share event
      trackShare(
        platformName as 'twitter' | 'facebook' | 'linkedin' | 'whatsapp' | 'copy',
        'tool', // Default to 'tool' as content type
        title, // Using title as content ID for now
        title,
        undefined
      );
      onShare?.(platformName);
      
      // Close dropdown/share sheet
      if (variant !== 'inline') {
        setIsOpen(false);
      }
    } catch (error) {
              ...
              <div className="p-2">
                {SHARE_PLATFORMS.map((platform) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-5 h-5 mr-3 ${platform.color}`} />
